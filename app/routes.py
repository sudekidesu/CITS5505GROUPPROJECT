...
def qa_detail(qa_id):
    if request.method == 'GET':
        ...
        return render_template("post-details.html", question_data=question_data, token=token, answers_data=answers_data)
    else:
...
def board():
    ...
    pagination = User.query.order_by(User.likes).paginate(page=page, per_page=per_page)
